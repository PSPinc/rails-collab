...
module Collab
  ...
  #
  module Models
    class TrackedPosition < ::Collab::Models::Base
      belongs_to :document, class_name: ::Collab.config.document_model
      belongs_to :owner, polymorphic: true

      validates :pos, presence: true,
                      numericality: { only_integer: true, greater_than_or_equal_to: 0 }
      validates :assoc, presence: true, inclusion: { in: [-1, 1] },
                        numericality: { only_integer: true }

      validates :deleted_at_version,
                numericality: { only_integer: true, greater_than_or_equal_to: 0 },
                if: :deleted_at_version

      scope :current, -> { where(deleted_at_version: nil) }

      # Resolves a set of positions and yields them to the block given,
      ...
      def self.resolve(document, *positions, version:)
        ...
        document.with_lock('FOR SHARE') do
          unless document.document_version == version
            steps = document.commits.where(
              'document_version > ?', @mapped_to
            ).order(document_version: :asc).pluck(:steps).flatten(1)

            map_results = ::Collab::JS.map_through(steps: steps, pos: positions)['pos']

            map_results.each_with_index do |row, idx|
              return false if row['deleted']

              positions[idx]['pos'] = row['pos']
            end
          end

          positions.map! do |pos|
            document.tracked_positions.current.find_or_initialize_by(
              pos: pos['pos'], assoc: pos['assoc']
            )
          end
